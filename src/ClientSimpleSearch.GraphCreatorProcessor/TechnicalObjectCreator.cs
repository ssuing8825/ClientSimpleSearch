...
{
    public class TechnicalObjectCreator : CreatorBase
    {
        private const string query = "dbo.GetTechnicalObjects";
        ...
        private string CreateSystems()
        {
            ...
            actionBuilder.Append("{\"query\" : \"CREATE (n:TechnicalObject, { props } )\",");
            ...
            if (reader.HasRows)
            {
                while (reader.Read())
                {
                    ...
                    actionBuilder.AppendFormat("\"Objectid\" : \"{0}\"", reader.GetString(0));
                    actionBuilder.Append("},");
